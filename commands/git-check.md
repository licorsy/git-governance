...
8. If branch protection is missing on any of `develop`/`staging`/`main` that exist in the repo, suggest `./scripts/setup-branch-protection.sh <owner>/<repo>` but don't run it without confirmation — it changes remote GitHub configuration.
