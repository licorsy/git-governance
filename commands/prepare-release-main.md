...
6. If the repo is tag-consumed, say so and state that the tag has **not** been cut yet. `main` moving without its tag is what `release-integrity` reports the next morning. This command stops at the PR: once the human has merged it, cutting the tag is a follow-up action the agent may still take without a separate confirmation, because the merge itself is what triggers it, not a fresh permission event — see `agents/git-governance-advisor.md`. `/promote-window` is the path that does the whole chain, merge and tag included, in one execution.
