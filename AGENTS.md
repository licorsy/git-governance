...
**This file is the source of truth for agent instructions.** `CLAUDE.md` imports
it with `@AGENTS.md` and states nothing of its own. `AGENTS.md` is the
tool-neutral convention — any coding agent that reads it gets the same policy
Claude Code does, with no second copy to keep in sync.
...
(`scripts/init-governance.sh` copies it as-is, together with the thin
`CLAUDE.md` — see "Replicating this setup" below). Do not add project-specific
detail here that wouldn't make sense verbatim in another repo.
